...
declareTaskLoop(changeLCDSensor);
...
void setup(){
  ...
  createTaskLoopWithStackSize(changeLCDSensor,LOW_PRIORITY,200); // a tarefa é criada com um Stack maior devido
  // ao uso de arrays no LCD
}
 ...
 *
 * Inicio uma região critica, visto que os comandos
 ...
 * sensor1 (readSensor1) ou sensor2 (readSensor2) identificar mudança no valor que foi 
 * lido do sensor, assim é chamado a macro resume(changeLCDSensor).
 * 
 */
taskLoop(changeLCDSensor){
  enterCritical();
  {
